...
void onlinePlotter(){
	...
	for (int j = 0; j < kNUM2DPLOTS; ++j)
	{
		...
		alltruth->Draw();
		alltruth->SetTitle(";Truth r;Event Counts");
		accepttruth->SetMarkerColor(kGreen);
		accepttruth->SetMarkerStyle(kOpenCircle);
		recoTrack->SetLineColor(kRed);
		recoVert->SetLineColor(kOrange);
		accepttruth->Draw("psame");
		recoTrack->Draw("same");
		recoVert->Draw("same");
		tlR->AddEntry(alltruth,"All Truth","l");
		tlR->AddEntry(accepttruth,"Truth in Acceptance","p");
		...
		alltruthpT->Draw();
		alltruthpT->SetTitle(";Truth #gamma pT;Event Counts");
		accepttruthpT->SetMarkerColor(kGreen);
		accepttruthpT->SetMarkerStyle(kOpenCircle);
		recoTrackpT->SetMarkerColor(kRed);
		recoTrackpT->SetMarkerStyle(kOpenTriangleDown);
		recoVertpT->SetLineColor(kOrange);
		accepttruthpT->Draw("psame");
		recoTrackpT->Draw("psame");
		recoVertpT->Draw("same");
		TLegend *tlpT = new TLegend(.4,.4,.6,.6);
		tlpT->AddEntry(alltruthpT,"All Truth","l");
		tlpT->AddEntry(accepttruthpT,"Truth in Acceptance","p");
		tlpT->AddEntry(recoTrackpT,"Reco Tracks","p");
