...
void makeFactory(TTree* signalTree,std::string outfile,std::string factoryname)
{
  ...
  factory->BookMethod(Types::kMLP,"MLP_ANN40","HiddenLayers=40:VarTransform=N:Sampling=.25:SamplingImportance=.5:ConvergenceTests=3:");
  factory->BookMethod(Types::kMLP,"MLP_ANN40L2","HiddenLayers=40,2:VarTransform=N:Sampling=.25:SamplingImportance=.5:ConvergenceTests=3:");
  factory->BookMethod(Types::kMLP,"MLP_ANN80L2","HiddenLayers=80,2:VarTransform=N:Sampling=.25:SamplingImportance=.5:ConvergenceTests=3:");
...
int vtxDNN(){
  ...
  string outname = "/direct/phenix+u/vassalli/sphenix/single/Training/condorout/vtxTrainDNN.root";
  unsigned int nFiles=200;

  TChain *signalTree = handleFile(treePath,treeExtension,"vtxingTree",nFiles);
  makeFactory(signalTree,outname,"vtxFactoryDNN");
